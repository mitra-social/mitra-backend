...
final class PrivateRouteProvider implements RouteProviderInterface
{
    public function __invoke(RouteCollectorProxyInterface $group): void
    {
        ...
        $group->post('/user/{username}/outbox', OutboxController::class)->setName('user-outbox-write');
        $group->get('/user/{username}/outbox', OutboxController::class)->setName('user-outbox-read');
